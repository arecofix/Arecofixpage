-- CONSOLIDATED POSTS SCHEMA AND DATA
-- This migration combines table creation, RLS setup, and initial data insertion.
...
-- 3. Create Policies (Drop existing to avoid conflicts)
...
-- Create policy for admins (authenticated users) to manage posts
CREATE POLICY "Admins can manage posts" ON posts
    FOR ALL
    USING (auth.role() = 'authenticated');

-- 4. Insert Content
-- Main Post: Servicio Técnico de Celulares en Marcos Paz
INSERT INTO posts (slug, title, content, published, image, meta_title, meta_description)
VALUES (
  'servicio-tecnico-de-celulares-en-marcos-paz', -- Corrected slug
