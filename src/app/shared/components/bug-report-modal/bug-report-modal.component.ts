...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in border border-slate-200 dark:border-slate-700"
      >
        <!-- Header -->
        <div
        ...
        <div class="p-6 space-y-5">
          @if (success) {
            <div
              class="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-lg text-emerald-800 dark:text-emerald-300 flex flex-col items-center justify-center py-8 text-center gap-3"
            >
              ...
              <button
                (click)="close.emit()"
                class="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md transition-colors"
              >
                Cerrar
              </button>
            </div>
            ...
            @if (error) {
              <div
                class="p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-md text-red-800 dark:text-red-300 text-sm flex items-center gap-2"
              >
            ...
            <div>
              <label
                ...
                >Área afectada (Opcional)</label
              >
              <select
                [(ngModel)]="category"
                class="w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-slate-300 dark:border-slate-700 rounded-md text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
              >
