...
@Component({
  ...
  template: `
    <div
      class="fixed top-20 right-2 z-9999 flex flex-col gap-2 pointer-events-none max-w-[85vw] sm:max-w-sm w-auto items-end"
    >
      @for (toast of toastService.toasts(); track toast.id) {
        <div
          class="pointer-events-auto transform transition-all duration-300 animate-slide-in-down cursor-pointer"
          (click)="onToastClick(toast)"
        >
          <div
            class="flex items-center gap-2 px-3 py-2 rounded-lg shadow-lg border-l-4 overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 ring-1 ring-black/5"
            [ngClass]="{
