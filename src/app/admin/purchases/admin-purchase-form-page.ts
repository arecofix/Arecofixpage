...
export class AdminPurchaseFormPage implements OnInit {
  ...
  updateNewProductField(
    field: keyof typeof this.newProductForm.prototype | string,
    value: any,
  ) {
  ...
  total = computed(() =>
    this.items().reduce((acc, item) => acc + item.quantity * item.unit_cost, 0),
  );
  ...
  async saveNewProduct() {
    ...
    try {
      ...
      this.error.set(
        'Error al crear producto rápido: ' +
