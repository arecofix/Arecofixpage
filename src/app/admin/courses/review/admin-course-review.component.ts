        ...
        <div class="grid gap-6">
          @for (course of pendingCourses(); track course.id) {
            <div class="bg-white p-6 rounded shadow border">
              ...
              <p class="text-sm text-gray-500 mb-4">Instructor ID: {{ course.author_id }}</p>
              ...
              <div class="mb-4">
                ...
                <a [href]="course.preview_video_url" target="_blank" class="text-blue-600 hover:underline">Ver Video</a>
              </div>

              <div class="mt-6 border-t pt-4">
                <label class="block text-sm font-medium mb-1">Feedback (opcional o si se rechaza):</label>
                <textarea [(ngModel)]="feedbacks[course.id]" class="w-full border rounded p-2 mb-4" rows="3"></textarea>
                
                <div class="flex space-x-3">
                  <button (click)="review(course.id, 'APPROVED')" class="bg-green-600 text-white px-4 py-2 rounded">Aprobar</button>
                  <button (click)="review(course.id, 'REJECTED')" class="bg-red-600 text-white px-4 py-2 rounded">Rechazar</button>
                </div>
              </div>
            </div>
          }
