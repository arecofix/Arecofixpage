  ...
  template: `
    <div class="max-w-6xl mx-auto pb-12">
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-slate-400 ml-11">
            Gestiona los recursos, videos y documentos para
            <strong>{{ course()?.title || 'Cargando...' }}</strong>
          </p>
        </div>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div class="lg:col-span-1 space-y-4">
            <h3
              class="font-bold text-gray-900 dark:text-white uppercase tracking-wider text-xs px-2"
            >
              Temario (Unidades)
            </h3>
            ...
            @if (modules().length === 0) {
              <div class="alert alert-warning text-sm rounded-xl">
                ...
                <span
                  >Este curso no tiene módulos creados. Edítalo primero para
                  agregar el temario.</span
                >
              </div>
            }

            <ul
              class="menu bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 w-full p-2 shadow-sm"
            >
                ...
                <li>
                  ...
                  >
                    ...
                    @if (isModuleUnlocked(mod)) {
                      <i
                        class="fas fa-circle text-emerald-400 text-[8px]"
                        title="Disponible"
                      ></i>
                    } @else if (mod.unlock_date) {
                      <i
                        class="fas fa-clock text-amber-400 text-xs"
                        title="Programado"
                      ></i>
                    } @else {
                      <i
                        class="fas fa-lock text-slate-300 dark:text-slate-600 text-xs"
                        title="Bloqueado"
                      ></i>
                    }
          ...
          <div class="lg:col-span-3">
            @if (selectedModule()) {
              ...
              >
                <div
                  class="bg-slate-50 dark:bg-slate-900/50 p-6 border-b border-slate-200 dark:border-slate-700"
                >
                  <!-- Module header row -->
                  <div class="flex justify-between items-center mb-5">
                    <div>
                      ...
                      <p class="text-sm text-slate-500 mt-1">
                        Organiza los recursos educativos para esta unidad.
                      </p>
                    </div>
                    <div class="dropdown dropdown-end">
                      ...
                      <ul
                        tabindex="0"
                        class="dropdown-content z-[1] menu p-2 shadow-xl bg-base-100 rounded-box w-52 border border-slate-200 dark:border-slate-700 mt-2"
                      >
                        <li>
                          <a (click)="addResource('video')"
                            ><i class="fas fa-video text-red-500"></i> Enlace de
                            Video (YouTube/Vimeo)</a
                          >
                        </li>
                        <li>
                          <a (click)="addResource('document')"
                            ><i class="fas fa-file-pdf text-orange-500"></i>
                            Documento / PDF</a
                          >
                        </li>
                        <li>
                          <a (click)="addResource('link')"
                            ><i class="fas fa-link text-blue-500"></i> Enlace
                            Externo (Meet, Web)</a
                          >
                        </li>
                        <li>
                          <a (click)="addResource('text')"
                            ><i class="fas fa-align-left text-green-500"></i>
                            Texto (Aviso / Info)</a
                          >
                        </li>
                        <li>
                          <a (click)="addResource('exam')"
                            ><i
                              class="fas fa-clipboard-list text-purple-500"
                            ></i>
                            Examen (Cuestionario)</a
                          >
                        </li>
                      </ul>
                    </div>
                  ...
                  <!-- ── DRIP CONTENT PANEL ────────────────────────────── -->
                  <div
                    class="rounded-xl border p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4"
                    [class.border-emerald-200]="
                      isModuleUnlocked(selectedModule())
                    "
                    [class.bg-emerald-50]="isModuleUnlocked(selectedModule())"
                    [class.dark:bg-emerald-900/10]="
                      isModuleUnlocked(selectedModule())
                    "
                    [class.dark:border-emerald-800]="
                      isModuleUnlocked(selectedModule())
                    "
                    [class.border-amber-200]="
                      !isModuleUnlocked(selectedModule()) &&
                      selectedModule()?.unlock_date
                    "
                    [class.bg-amber-50]="
                      !isModuleUnlocked(selectedModule()) &&
                      selectedModule()?.unlock_date
                    "
                    [class.dark:bg-amber-900/10]="
                      !isModuleUnlocked(selectedModule()) &&
                      selectedModule()?.unlock_date
                    "
                    [class.dark:border-amber-800]="
                      !isModuleUnlocked(selectedModule()) &&
                      selectedModule()?.unlock_date
                    "
                  ...
                  >
                    ...
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                      @if (isModuleUnlocked(selectedModule())) {
                        <div
                          class="w-9 h-9 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0"
                        >
                          <i class="fas fa-lock-open text-sm"></i>
                        </div>
                        <div>
                          <p
                            class="font-bold text-emerald-800 dark:text-emerald-300 text-sm"
                          >
                        ...
                        <div
                          class="w-9 h-9 rounded-full bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0"
                        >
                          <i class="fas fa-clock text-sm"></i>
                        </div>
                        <div>
                          <p
                            class="font-bold text-amber-800 dark:text-amber-300 text-sm"
                          >
                            Programado
                          </p>
                        ...
                        <div
                          class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 shrink-0"
                        >
                          <i class="fas fa-lock text-sm"></i>
                        </div>
                        <div>
                          <p
                            class="font-bold text-slate-700 dark:text-slate-300 text-sm"
                          >
                            Bloqueado
                          </p>
                          <p class="text-xs text-slate-400">
                            Sin fecha programada — los alumnos no pueden acceder
                            al contenido
                          </p>
                        </div>
                      }
                    ...
                    <div class="flex items-center gap-2 flex-wrap shrink-0">
                      ...
                      <input
                        ...
                        [value]="
                          selectedModule()?.unlock_date
                            ? selectedModule()!.unlock_date!.slice(0, 16)
                            : ''
                        "
                      ...
                      <!-- Enable now -->
                      <button
                        class="btn btn-sm btn-success rounded-xl gap-1"
                        [disabled]="
                          updatingDrip() || isModuleUnlocked(selectedModule())
                        "
                        (click)="enableNow()"
                      ...
                      <!-- Lock -->
                      <button
                        class="btn btn-sm btn-ghost text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-xl gap-1"
                        [disabled]="
                          updatingDrip() || !selectedModule()?.unlock_date
                        "
                        (click)="lockModule()"
                ...
                <div class="p-6 space-y-4 bg-slate-50 dark:bg-slate-800/50">
                  @if (loadingContents()) {
                    <div class="flex justify-center py-10">
                      <span
                        class="loading loading-spinner text-blue-500"
                      ></span>
                    </div>
                  } @else if (contents().length === 0) {
                    <div
                      class="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl bg-white dark:bg-slate-800"
                    >
                      <i
                        class="fas fa-box-open text-4xl text-slate-300 dark:text-slate-600 mb-3"
                      ></i>
                    ...
                    @for (content of contents(); track content.id || $index) {
                      <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-start shadow-sm transition-all hover:border-blue-300"
                      >
                        <!-- Icon based on type -->
                        ...
                        <div class="grow w-full space-y-3">
                          ...
                          <div class="form-control w-full">
                            <input
                              ...
                              class="input input-sm border-slate-200 dark:border-slate-700 focus:border-blue-500 w-full font-bold text-gray-900 dark:text-white"
                          ...
                          @if (content.type === 'document') {
                            <div class="flex items-center gap-2">
                              <input
                                type="text"
                                [(ngModel)]="content.url"
                                class="input input-sm border-slate-200 dark:border-slate-700 text-xs w-full text-slate-500"
                                placeholder="URL del archivo (o súbelo usando el botón)"
                              />

                              <label
                                class="btn btn-sm btn-outline rounded-lg flex-none cursor-pointer"
                              >
                                ...
                                <input
                                  ...
                                  (change)="uploadFile($event, content)"
                                />
                              </label>
                            </div>
                            ...
                            <textarea
                              [(ngModel)]="content.url"
                              class="textarea textarea-bordered border-slate-200 dark:border-slate-700 w-full text-sm h-24"
                            ...
                            <div class="flex items-center gap-3">
                              <div class="text-sm text-slate-500">
                                <strong>{{
                                  content.metadata?.questions?.length || 0
                                }}</strong>
                                preguntas configuradas. Puntaje para aprobar:
                                <strong
                                  >{{
                                    content.metadata?.passing_score || 60
                                  }}%</strong
                                >
                              </div>
                              <button
                                class="btn btn-sm btn-primary rounded-lg"
                                (click)="openExamEditor(content)"
                              >
                            ...
                            <input
                              type="text"
                              [(ngModel)]="content.url"
                              class="input input-sm border-slate-200 dark:border-slate-700 w-full text-sm text-blue-500"
                              placeholder="https://..."
                            />
                          }
                        </div>
                        ...
                        <div class="flex-none flex items-center gap-1">
                          <!-- Up/Down Ordering -->
                          <div
                            class="flex flex-col border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden"
                          >
                            <button
                              class="btn btn-xs btn-ghost rounded-none h-6 px-2 hover:bg-slate-100"
                              (click)="moveUp($index)"
                              [disabled]="$index === 0"
                            >
                            ...
                            <div
                              class="h-[1px] bg-slate-100 dark:bg-slate-700"
                            ></div>
                            <button
                              class="btn btn-xs btn-ghost rounded-none h-6 px-2 hover:bg-slate-100"
                          ...
                          <button
                            class="btn btn-square btn-ghost text-red-500 hover:bg-red-50"
              ...
              <div
                class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 h-[400px] flex flex-col items-center justify-center text-slate-400"
              >
                <i class="fas fa-hand-pointer text-5xl mb-4 opacity-20"></i>
                <p class="font-medium text-lg">
                  Selecciona un Módulo del Temario
                </p>
                <p class="text-sm mt-1">
                  Para visualizar y gestionar sus recursos educativos.
                </p>
              </div>
            }
          </div>
      ...
      <dialog
        ...
        [class.modal-open]="editingExamContent !== null"
      >
        <div
          class="modal-box max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xl"
        >
          <button
            class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
            (click)="closeExamEditor()"
          >
          ...
          @if (editingExamContent) {
            <div class="form-control mb-6">
              <label class="label"
                ><span class="label-text font-bold"
                  >Puntaje mínimo para aprobar (%)</span
                ></label
              >
              <input
                type="number"
                [(ngModel)]="editingExamContent.metadata.passing_score"
            ...
            <div class="divider">
              Preguntas ({{
                editingExamContent.metadata.questions?.length || 0
              }})
            </div>

            <div class="space-y-6">
              @for (
                q of editingExamContent.metadata.questions;
                track $index;
                let qIndex = $index
              ) {
                <div
                  class="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 relative"
                >
                  <button
                    class="btn btn-sm btn-square btn-ghost text-red-500 absolute top-2 right-2"
                  ...
                  <label class="label py-0 mb-2"
                    ><span class="label-text text-xs text-slate-500"
                      >Opciones (Marca la correcta)</span
                    ></label
                  >
                    ...
                    @for (
                      ...
                      <div class="flex items-center gap-3">
                        <input
                          ...
                          [value]="optIndex"
                          [(ngModel)]="q.correct_option_index"
                        ...
                        <input
                          ...
                          (ngModelChange)="
                            trackOptionChange(q, optIndex, $event)
                          "
            ...
            <div class="mt-6">
              <button
                ...
                (click)="addQuestion()"
              >
                <i class="fas fa-plus"></i> Añadir Nueva Pregunta
              </button>
            </div>

            <div class="modal-action mt-8">
              <button class="btn btn-primary px-8" (click)="closeExamEditor()">
                Listo
              </button>
            </div>
          }
...
export class AdminCourseMaterialsPage implements OnInit {
  ...
  loadData() {
    ...
    // Load Modules
    this.coursesService
      .getModulesByCourseId(this.courseId())
      .subscribe((res) => {
  ...
  enableNow() {
    ...
    this.coursesService
      .updateModuleUnlockDate(mod.id, 'now')
      .subscribe((res) => {
        ...
        if (res.error) {
          ...
          this.notification.showSuccess(
            '✅ Módulo habilitado para los alumnos.',
          );
        }
      });
  }
  ...
  lockModule() {
    ...
    if (
      !confirm(
        '¿Querés bloquear este módulo? Los alumnos dejarán de ver su contenido.',
      )
    )
      return;
    this.updatingDrip.set(true);
    this.coursesService
      .updateModuleUnlockDate(mod.id, null)
      .subscribe((res) => {
        this.updatingDrip.set(false);
        if (res.error) {
  ...
  scheduleUnlock(event: Event) {
    ...
    this.updatingDrip.set(true);
    this.coursesService
      .updateModuleUnlockDate(mod.id, isoDate)
      .subscribe((res) => {
        this.updatingDrip.set(false);
        if (res.error) {
          ...
          this.notification.showSuccess(
            `📅 Módulo programado: ${this.formatUnlockDate(updated.unlock_date)}`,
          );
        }
      });
  }
  ...
  async uploadFile(event: any, contentRef: ModuleContent) {
    ...
    try {
      ...
      const publicUrl = await this.mediaService.uploadFile(
        file,
        'course-materials',
      );
      ...
      this.contents.update((current) => {
        ...
        if (index !== -1) {
          ...
          if (
            updated[index].title === 'Nuevo Documento' ||
            !updated[index].title
          ) {
  ...
  async saveContents() {
    ...
    for (const c of currentContents) {
      if (!c.title || (!c.url && c.type !== 'exam')) {
        this.notification.showError(
          'Todos los recursos deben tener Título y Contenido/URL.',
        );
        return;
      }
    }
    ...
    try {
      const res = await firstValueFrom(
        this.coursesService.saveModuleContents(modId, currentContents),
      );
      ...
      for (let i = 0; i < savedContents.length; i++) {
        ...
        if (savedItem.type === 'exam' && savedItem.metadata?.questions) {
          const questionsPayload = savedItem.metadata.questions.map(
            (q: any, idx: number) => ({
          ...
          const qRes = await firstValueFrom(
            this.coursesService.saveExamQuestions(
              savedItem.id,
              questionsPayload,
            ),
          );
          if (qRes?.error) {
            ...
            this.notification.showError(
              'Materiales guardados, pero falló al guardar las preguntas del examen.',
            );
      ...
      this.notification.showSuccess(
        'Materiales y exámenes guardados correctamente.',
      );
      this.contents.set(savedContents);
    } catch (err: any) {
      this.notification.showError(
        'Error al guardar materiales: ' + err.message,
      );
    } finally {
      this.saving.set(false);
    }
  ...
  async openExamEditor(content: ModuleContent) {
    if (!content.metadata)
      content.metadata = { passing_score: 60, questions: [] };

    if (content.id) {
      try {
        const res = await firstValueFrom(
          this.coursesService.getExamQuestions(content.id),
        );
        if (res.data && res.data.length > 0) {
          // Ensure correct format and types
          content.metadata.questions = res.data
