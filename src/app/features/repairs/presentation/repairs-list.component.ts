...
import { RepairsRepository, RepairListDto } from '../infrastructure/repositories/repairs.repository';
...
@Component({
  ...
  template: `
    <div class="optimization-container min-h-screen bg-slate-950 text-slate-100 font-sans p-6">
      <!-- Encabezado de la página -->
      <header class="max-w-6xl mx-auto mb-8 animate-fade-in">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 class="text-3xl font-extrabold bg-gradient-to-r from-emerald-400 to-cyan-500 bg-clip-text text-transparent">
              Panel de Reparaciones Optimizado
            </h1>
            <p class="text-sm text-slate-400 mt-1">
              Arquitectura Clean para mitigar el consumo de <strong>Cached Egress</strong> de Supabase.
            </p>
          </div>
          <!-- Resumen de Optimización -->
          <div class="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/50 flex gap-4 text-xs">
            <div>
      ...
      <section class="max-w-6xl mx-auto mb-8 glass-card p-5 rounded-2xl border border-slate-800 bg-slate-900/30">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <!-- Búsqueda -->
          <div class="relative">
            <label class="block text-xs text-slate-400 mb-1 font-semibold">Buscar Reparación</label>
            <input 
              type="text" 
              [ngModel]="searchTerm()" 
          ...
          <!-- Sucursal -->
          <div>
            <label class="block text-xs text-slate-400 mb-1 font-semibold">Filtrar por Sucursal</label>
            <select 
              [ngModel]="branchId()" 
          ...
          <div class="border border-dashed border-slate-700 hover:border-cyan-500 rounded-xl p-3 flex flex-col justify-center items-center cursor-pointer relative bg-slate-900/20 transition-all group">
            <input 
              type="file" 
              accept="image/*" 
            ...
            <div class="text-center" *ngIf="!isUploading() && !uploadSuccess()">
              <p class="text-xs font-semibold text-slate-300 group-hover:text-cyan-400 transition-colors">
                📸 Probar Subida Comprimida
              </p>
              <p class="text-[10px] text-slate-500 mt-0.5">Optimiza WebP antes de Supabase</p>
            </div>
            
            <div class="text-center" *ngIf="isUploading()">
              <div class="animate-spin rounded-full h-4 w-4 border-2 border-cyan-500 border-t-transparent mx-auto mb-1"></div>
              <p class="text-[10px] text-cyan-400">{{ uploadStatus() }}</p>
            </div>

            <div class="text-center" *ngIf="uploadSuccess()">
              <p class="text-xs font-bold text-emerald-400">✓ ¡Imagen Subida!</p>
              <p class="text-[9px] text-slate-400 mt-0.5">Ahorro: {{ compressionSavings() }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <main class="max-w-6xl mx-auto glass-card rounded-2xl border border-slate-800 bg-slate-900/20 overflow-hidden">
              ...
              @for (repair of repairsList(); track repair.id) {
                <tr class="hover:bg-slate-900/40 transition-colors group">
                  ...
                  <td class="py-4 px-6 flex items-center gap-3">
                    <div class="relative w-10 h-10 rounded-lg overflow-hidden bg-slate-800 border border-slate-700">
                      <!-- 
                    ...
                    <div>
                      <span class="font-bold text-slate-200 group-hover:text-emerald-400 transition-colors">
                        {{ repair.device_brand }}
                      </span>
                      <span class="block text-xs text-slate-400">{{ repair.device_model }}</span>
                    </div>
                  </td>
                  ...
                  <td class="py-4 px-6 font-mono text-xs text-slate-400">
                    <span class="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-md">
                ...
                <tr>
                  <td colspan="6" class="text-center py-12 text-slate-500">
                    @if (store.loading()) {
                      ...
                      <p class="text-base text-slate-400 font-semibold mb-1">No se encontraron reparaciones</p>
                      <p class="text-xs text-slate-500">Prueba ajustando los filtros o el término de búsqueda.</p>
                    }
                  </td>
                </tr>
              }
        ...
        <div class="py-6 border-t border-slate-800/80 bg-slate-900/10 flex justify-between items-center px-6">
          <p class="text-xs text-slate-500">
            Mostrando <strong class="text-slate-300">{{ repairsList().length }}</strong> de 
            <strong class="text-slate-300">{{ totalCount() }}</strong> reparaciones
          </p>
...
export class RepairListComponent implements OnInit {
  ...
  handleImageError(event: any): void {
    ...
    event.target.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="%23475569" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="20" x="5" y="2" rx="2" ry="2"/><path d="M12 18h.01"/></svg>';
  }
