...
export class SupabaseProductStockRepository implements ProductStockRepository {
  ...
  async updateBranchStock(productId: string, branchId: string, quantity: number, tenantId: string | null): Promise<void> {
  ...
  async deductStock(productId: string, branchId: string, quantityToDeduct: number, tenantId: string | null): Promise<void> {
    ...
    if (error) {
       console.error('[SupabaseProductStockRepository] RPC Failed:', error);
