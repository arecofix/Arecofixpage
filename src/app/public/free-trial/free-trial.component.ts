...
export class FreeTrialPageComponent implements OnInit {
  ...
  exeDownloadUrl =
    'https://github.com/arecofix/Arecofixpage/releases/latest/download/Arecofix-setup.exe'; // Reemplazar luego con la URL real
  ...
  private checkHardwareLock() {
    if (isPlatformBrowser(this.platformId)) {
      if (localStorage.getItem('arecofix_trial_claimed') === 'true') {
        ...
        this.errorMessage =
          'Ya has registrado una prueba gratuita de 72 h en este dispositivo. Para continuar usando el sistema, adquiere una licencia definitiva.';
      }
    }
  }
  ...
  async onSubmit() {
    ...
    try {
      ...
      this.successMessage =
        '¡Prueba creada! Iniciando sesión y preparando tu descarga...';
      ...
      const { error: signInError, data: signInData } = await this.supabase
        .getClient()
      ...
      if (signInError) {
        throw new Error(
          'Cuenta creada, pero hubo un error al iniciar sesión. Por favor, revisa tu correo electrónico.',
        );
      }
