  ...
  template: `
    <div class="min-h-screen bg-slate-50 dark:bg-slate-900 pb-20">
      @if (loading()) {
        ...
        <div class="flex justify-center items-center h-[60vh] flex-col text-slate-500">
          <i class="fas fa-exclamation-circle text-4xl mb-4"></i>
          <h2 class="text-xl font-bold">Curso no encontrado</h2>
          <button routerLink="/academy" class="btn btn-primary mt-4">Volver a la Academia</button>
        </div>
      } @else if (!hasAccess()) {
        <div class="flex flex-col justify-center items-center h-[60vh] max-w-lg mx-auto text-center px-4">
          <div class="w-20 h-20 bg-orange-100 text-orange-500 rounded-full flex items-center justify-center text-3xl mb-4">
            <i class="fas fa-lock"></i>
          </div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Acceso Restringido</h2>
          <p class="text-slate-500 mb-6">No tienes una inscripción activa para este curso o aún está pendiente de aprobación.</p>
          <button [routerLink]="['/academy', course()?.slug]" class="btn btn-primary w-full sm:w-auto">Ir a la página del Curso</button>
        </div>
      } @else {
        ...
        <div class="bg-blue-900 dark:bg-slate-950 text-white relative overflow-hidden">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-blue-900/90 to-transparent dark:from-slate-950"></div>
            
            <div class="max-w-6xl mx-auto px-4 py-12 relative z-10">
                <div class="flex flex-col md:flex-row gap-6 items-end justify-between">
                    <div>
                        <div class="badge badge-primary badge-sm font-bold mb-3 uppercase tracking-widest border-none bg-blue-500 text-white">{{ course()?.level === 'basic' ? 'Básico' : course()?.level === 'intermediate' ? 'Intermedio' : 'Avanzado' }}</div>
                        <h1 class="text-3xl md:text-5xl font-black mb-2 leading-tight">{{ course()?.title }}</h1>
                        <div class="flex items-center gap-4 text-sm text-blue-200 font-medium">
                            @if (course()?.instructor_name) {
                                <span class="flex items-center gap-2"><i class="fas fa-chalkboard-teacher text-blue-400"></i> Prof. {{ course()?.instructor_name }}</span>
                            }
                            <span class="flex items-center gap-2"><i class="fas fa-layer-group text-blue-400"></i> {{ modules().length }} Módulos</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-6xl mx-auto px-4 py-8">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div class="lg:col-span-3 space-y-6">
                    ...
                    @if (course()?.description) {
                        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
                            <h3 class="font-bold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><i class="fas fa-bullhorn text-orange-500"></i> Información del Curso</h3>
                            <p class="text-slate-600 dark:text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{{ course()?.description }}</p>
                        </div>
                    }

                    <!-- Modules Accordion -->
                    <h3 class="font-black text-xl text-gray-900 dark:text-white mt-8 mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">Temario y Recursos</h3>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <!-- Instructor -->
                    @if (course()?.instructor_name) {
                        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 text-center">
                            <div class="w-20 h-20 mx-auto rounded-full overflow-hidden mb-3 border-4 border-slate-50 dark:border-slate-700">
                                <img [src]="getImageSrc(course()?.instructor_avatar)" onerror="this.src='/assets/img/avatar.png'" class="w-full h-full object-cover" />
                            </div>
                            <h3 class="font-bold text-gray-900 dark:text-white">{{ course()?.instructor_name }}</h3>
                            <p class="text-xs text-slate-500 mb-4">{{ course()?.instructor_role || 'Profesor(a)' }}</p>
                            <a [href]="'mailto:info@arecofix.com.ar'" class="btn btn-sm btn-outline w-full rounded-xl"><i class="fas fa-envelope"></i> Contactar</a>
                        </div>
                    }
                </div>
      ...
      <!-- Exam Modal -->
      <dialog id="take_exam_modal" class="modal modal-bottom sm:modal-middle" [class.modal-open]="activeExam !== null">
          <div class="modal-box max-w-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xl">
              @if (activeExam) {
                  <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" (click)="closeExam()"><i class="fas fa-times"></i></button>
                  <h3 class="font-black text-2xl mb-2 text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-4">
                      {{ activeExam.title }}
                  </h3>
                  ...
                  @if (examLoading()) {
                      ...
                      <div class="text-center py-10">
                          <div class="w-24 h-24 mx-auto rounded-full flex items-center justify-center text-4xl mb-4"
                               [class.bg-green-100]="examResult().passed" [class.text-green-600]="examResult().passed"
                               [class.bg-red-100]="!examResult().passed" [class.text-red-600]="!examResult().passed">
                              <i class="fas" [class.fa-check]="examResult().passed" [class.fa-times]="!examResult().passed"></i>
                          </div>
                          ...
                          <p class="text-lg text-slate-600 dark:text-slate-400 mb-6">
                              Obtuviste <strong class="text-gray-900 dark:text-white">{{ examResult().score | number:'1.0-0' }}%</strong>
                              <br>
                              ({{ examResult().correct_answers }} de {{ examResult().total_questions }} correctas)
                          </p>
                          <button class="btn btn-primary" (click)="closeExam()">Continuar</button>
                      </div>
                  } @else {
                      <!-- Exam Questions -->
                      <div class="space-y-8 mt-6">
                          @for (q of activeExamQuestions(); track q.id; let qIndex = $index) {
                              <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                                  <h4 class="font-bold text-lg text-gray-900 dark:text-white mb-4">
                                      <span class="text-blue-500 mr-2">{{ qIndex + 1 }}.</span> {{ q.question_text }}
                                  </h4>
                                  <div class="space-y-3">
                                      @for (opt of q.options; track optIndex; let optIndex = $index) {
                                          <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-600 hover:bg-white dark:hover:bg-slate-700 cursor-pointer transition-colors"
                                                 [class.bg-blue-50]="activeExamAnswers[q.id] === optIndex" [class.border-blue-300]="activeExamAnswers[q.id] === optIndex"
                                                 [class.dark:bg-blue-900]="activeExamAnswers[q.id] === optIndex" [class.dark:border-blue-500]="activeExamAnswers[q.id] === optIndex">
                                              <input type="radio" [name]="'q_' + q.id" [value]="optIndex" [(ngModel)]="activeExamAnswers[q.id]" class="radio radio-primary">
                                              <span class="text-slate-700 dark:text-slate-300 font-medium">{{ opt }}</span>
                                          </label>
                                      }
                                  </div>
                              </div>
                      ...
                      <div class="modal-action mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
                          <button class="btn btn-outline" (click)="closeExam()">Cancelar</button>
                          <button class="btn btn-primary px-8" (click)="submitExam()" [disabled]="submittingExam()">
                              @if (submittingExam()) { <span class="loading loading-spinner"></span> }
                              Entregar Examen
                          </button>
                      </div>
                  }
          ...
          <form method="dialog" class="modal-backdrop" (click)="closeExam()"><button>close</button></form>
      </dialog>
    </div>
  `
})
export class StudentCampusPage implements OnInit {
  ...
  getTypeLabel(type: string): string {
      switch(type) {
          ...
          case 'document': return 'Material de Estudio';
  ...
  submitExam() {
      ...
      for (const q of questions) {
          if (this.activeExamAnswers[q.id] === undefined) {
              alert('Por favor, responde a todas las preguntas antes de entregar el examen.');
              return;
          }
      }
      ...
      this.coursesService.submitExam(this.activeExam.id!, payload).subscribe(res => {
