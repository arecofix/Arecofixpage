...
import {
  Course,
  CourseLevel,
} from '@app/features/courses/domain/entities/course.entity';
...
export class CreateCourseComponent {
  ...
  async submitCourse() {
    ...
    if (!this.course.title || !this.course.description || !this.course.price) {
      this.error.set(
        'Por favor completa todos los campos requeridos (Título, Descripción, Precio).',
      );
      return;
    }
    ...
    try {
      ...
      // @ts-ignore
      const { data, error } = await this.coursesService.supabase
        .from('courses')
        .insert(payload);
