...
import {
  Repair,
  RepairStatus,
} from '../../features/repairs/domain/entities/repair.entity';
...
export class TrackingPage implements OnInit, OnDestroy {
  @ViewChild('accessoryCarousel')
  accessoryCarousel?: ElementRef<HTMLDivElement>;
  ...
  trackingSteps = [
    {
      ...
      label: 'Proceso de Diagnóstico',
      icon: 'fas fa-microscope',
    },
    {
      id: 2,
      shortLabel: 'Repuestos',
      label: 'Logística de Repuestos',
      icon: 'fas fa-boxes',
    },
    {
      id: 3,
      ...
      if (this.code && this.code !== 'CONSULTA') {
        ...
        if (repairData) {
          ...
          if (isPlatformBrowser(this.platformId)) {
            if (
              !repairData.glass_upsell &&
              !localStorage.getItem(`upsellDismissed_${this.code}`)
            ) {
      ...
      next: (repairData) => {
        if (repairData) {
          ...
          if (isPlatformBrowser(this.platformId)) {
            if (
              !repairData.glass_upsell &&
              !localStorage.getItem(`upsellDismissed_${code}`)
            ) {
              setTimeout(() => this.showUpsellModal.set(true), 2500);
            }
          }
  ...
  async printTicket() {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(`TALÓN DE RETIRO # ${r.repair_number || 'S/N'}`, 40, y, {
      align: 'center',
    });
    ...
    doc.text(`Fecha:`, 5, y);
    doc.setFont('helvetica', 'bold');
    doc.text(`${new Date(r.received_at).toLocaleDateString()}`, 75, y, {
      align: 'right',
    });
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(`${(r.customer_name || 'Particular').toUpperCase()}`, 75, y, {
      align: 'right',
    });
    ...
    y += 8;
    doc.setFontSize(16);
    doc.text(`$ ${r.balance_to_pay.toLocaleString('es-AR')}`, 40, y, {
      align: 'center',
    });
    ...
    y += 3;
    doc.text(
      '3. Equipos no retirados en 60 días se consideran en abandono.',
      5,
      y,
    );
    y += 3;
    ...
    doc.save(
      `Arecofix_Talon_${r.repair_number || 'S-N'}_${r.tracking_code}.pdf`,
    );
  }
  ...
  async acceptUpsell() {
    ...
    try {
      ...
      const r = this.repair();
      const identifier = r?.repair_number
        ? `#${r.repair_number}`
        : r?.tracking_code || this.code;
      const msg = encodeURIComponent(
        `Hola! Quiero incluir un vidrio templado con mi reparación del equipo ${identifier}`,
      );
  ...
  async loadRecommendations(code: string) {
    try {
      ...
      const { data, error } =
        await this.trackingService.getRecommendedAccessories(code, brand);
      if (!error && data) {
  ...
  async buyAccessory(product: any) {
    ...
    try {
      const { error } = await this.trackingService.addAccessoryUpsell(
        this.code,
        product.id,
      );
      ...
      const identifier = r?.repair_number
        ...
        : r?.tracking_code || this.code;
      const msg = encodeURIComponent(
        `Hola! Acabo de agregar a mi reparación (Orden ${identifier}) el accesorio: "${product.name}". ¡Gracias!`,
      );
      window.open(`https://wa.me/${this.whatsappNumber}?text=${msg}`, '_blank');
    } catch (e) {
      this.logger.error('Failed to buy accessory', e);
      alert(
        'No pudimos procesar la solicitud en este momento. Inténtelo más tarde.',
      );
    } finally {
