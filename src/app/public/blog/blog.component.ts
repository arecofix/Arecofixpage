...
export class BlogComponent {
  ...
  async ngOnInit() {
    this.seoService.setPageData({
        ...
        description: 'Artículos, noticias y tutoriales sobre reparación de celulares, tablets, microsoldadura y tecnología. Espacio abierto para la comunidad técnica.',
    ...
    try {
      ...
      this.officialPosts.set(publicPosts.filter(p => p.author_role === 'admin' || p.author_role === 'super_admin' || p.author_role === 'tenant_owner'));
      this.communityPosts.set(publicPosts.filter(p => p.author_role !== 'admin' && p.author_role !== 'super_admin' && p.author_role !== 'tenant_owner'));
    } catch (err) {
