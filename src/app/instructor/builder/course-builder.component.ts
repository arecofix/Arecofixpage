...
export class CourseBuilderComponent implements OnInit {
    ...
    try {
      ...
      if (UUID_RE.test(idOrSlug)) {
        const res = await firstValueFrom(
          this.coursesService.getCourseById(idOrSlug),
        );
        if (res.error || !res.data) throw new Error('Curso no encontrado');
        course = res.data;
        if (course.slug) {
          this.router.navigate(['/instructor/builder', course.slug], {
            replaceUrl: true,
          });
        }
      } else {
        const res = await firstValueFrom(
          this.coursesService.getCourseBySlug(idOrSlug),
        );
        if (res.error || !res.data) throw new Error('Curso no encontrado');
        course = res.data;
      }
      ...
      this.courseData = {
        ...course,
        audience_list: Array.isArray(course.audience_list)
          ? [...course.audience_list]
          : [],
      };
      const modulesRes = await firstValueFrom(
        this.coursesService.getModulesByCourseId(course.id),
      );
      if (modulesRes.data) {
        this.modules = modulesRes.data.map((m: Module) => ({
          ...m,
          uploading: false,
          uploadProgress: 0,
        }));
        for (const mod of this.modules) {
          const contentsRes = await firstValueFrom(
            this.coursesService.getModuleContents(mod.id),
          );
      ...
      this.errorMsg.set(
        e instanceof Error ? e.message : 'Error al cargar el curso',
      );
    } finally {
      this.loading.set(false);
    }
  ...
  addAudienceItem() {
    ...
    if (!trimmed) return;
    if (!Array.isArray(this.courseData.audience_list))
      this.courseData.audience_list = [];
  ...
  removeAudienceItem(index: number) {
    ...
    this.courseData.audience_list = this.courseData.audience_list.filter(
      (_, i) => i !== index,
    );
  }
  ...
  moveModule(index: number, direction: number) {
    if (index + direction < 0 || index + direction >= this.modules.length)
      return;
  ...
  async onDrop(
    event: DragEvent,
    mod: { id: string; uploading: boolean; uploadProgress: number },
  ) {
    event.preventDefault();
  ...
  removeContent(modId: string, contentId: string) {
    this.moduleContentsMap[modId] = this.moduleContentsMap[modId].filter(
      (c) => c.id !== contentId,
    );
  }
    ...
    try {
      if (this.courseId()) {
        await firstValueFrom(
          this.coursesService.updateCourse(this.courseId()!, this.courseData),
        );
        const cleanModules = this.modules.map(
          ({ uploading, uploadProgress, ...rest }) => rest,
        );
        const savedModulesRes = await firstValueFrom(
          this.coursesService.saveModules(this.courseId()!, cleanModules),
        );
        ...
        if (res.data) {
          ...
          this.router.navigate(['/instructor/builder', slug], {
            replaceUrl: true,
          });
        }
      }
      ...
      this.errorMsg.set(
        e instanceof Error ? e.message : 'Error al guardar el curso',
      );
    } finally {
