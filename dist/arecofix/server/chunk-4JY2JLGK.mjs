...
var AdminPurchaseFormPage = class _AdminPurchaseFormPage {
  ...
  async ngOnInit() {
    ...
    const { data: suppliersRes } = await supabase.from("suppliers").select("*").eq("is_active", true).eq("tenant_id", tenantId);
    if (suppliersRes)
      this.suppliers.set(suppliersRes);
  ...
  async save() {
    ...
    try {
      ...
      const { data: purchase, error: purchaseError } = await supabase.from("purchases").insert({
        ...
        // fixed mapped property per schema
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AdminPurchaseFormPage, [{
    type: Component,
    args: [{ selector: "app-admin-purchase-form-page", standalone: true, imports: [FormsModule, RouterLink], template: `<div class="max-w-4xl mx-auto text-base-content">\r
          ...
          @for (item of items(); track $index) {\r
            <div class="flex gap-4 items-end bg-base-200 p-4 rounded text-base-content">\r
              <div class="form-control flex-1">\r
                @if ($index === 0) {\r
                  <label class="label text-xs text-base-content/70">Producto</label>\r
                }\r
              ...
              <div class="form-control w-24">\r
                @if ($index === 0) {\r
                  <label class="label text-xs text-base-content/70">Cantidad</label>\r
                }\r
                ...
                <div class="form-control w-32">\r
                  @if ($index === 0) {\r
                    <label class="label text-xs text-base-content/70">Costo Unit.</label>\r
                  }\r
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminPurchaseFormPage, { className: "AdminPurchaseFormPage", filePath: "src/app/admin/purchases/admin-purchase-form-page.ts", lineNumber: 15 });
})();
export {
  AdminPurchaseFormPage
};
//# sourceMappingURL=chunk-4JY2JLGK.mjs.map
