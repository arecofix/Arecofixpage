...
import {
  CategoryRepository
} from "./chunk-WPRKDIZO.mjs";
import {
  BrandRepository
} from "./chunk-XL3PKKWP.mjs";
import {
  ToastService
} from "./chunk-GHI7DVCX.mjs";
import {
  ProductRepository
...
var ServerXhr = class _ServerXhr {
  ...
  async \u0275loadImpl() {
    if (!this.xhrImpl) {
      const {
        default: xhr
      } = await import("./chunk-FPDZ2CKT.mjs");
      this.xhrImpl = xhr;
    }
  }
  build() {
...
var ServerPlatformLocation = class _ServerPlatformLocation {
  ...
  constructor() {
    const config3 = inject(INITIAL_CONFIG, {
      optional: true
    });
    if (!config3) {
      return;
    ...
    if (config3.url) {
      const url = parseUrl(config3.url, this._doc.location.origin);
...
function _document() {
  ...
  if (config3 && config3.document) {
    document2 = typeof config3.document === "string" ? _enableDomEmulation ? parseDocument(config3.document, config3.url) : window.document : config3.document;
  } else {
  ...
  function fromJSON(json, inputs) {
    ...
    if (ownInputs) {
      ...
      for (let input2 of ownInputs) {
        let inputHydrated = __spreadProps(__spreadValues({}, input2), { __proto__: Input2.prototype });
...
async function renderAngular(html, bootstrap2, url, platformProviders, serverContext) {
  ...
  try {
    ...
    if (isNgModule(bootstrap2)) {
      const moduleRef = await platformRef.bootstrapModule(bootstrap2);
      applicationRef = moduleRef.injector.get(ApplicationRef);
    } else {
      applicationRef = await bootstrap2({
        platformRef
      });
    }
    await applicationRef.whenStable();
...
async function getRoutesFromAngularRouterConfig(bootstrap2, document2, url, invokeGetPrerenderParams = false, includePrerenderFallbackRoutes = true, entryPointToBrowserMapping = void 0) {
  const {
    protocol,
    host
  } = url;
...
function extractRoutesAndCreateRouteTree(options) {
  ...
  async function extract() {
    ...
    const bootstrap2 = await manifest.bootstrap();
    const {
      baseHref,
      appShellRoute,
      routes: routes2,
      errors
    } = await getRoutesFromAngularRouterConfig(bootstrap2, document2, url, invokeGetPrerenderParams, includePrerenderFallbackRoutes, manifest.entryPointToBrowserMapping);
...
var AngularServerApp = class {
  ...
  async handleRendering(request, matchedRoute, requestContext) {
    ...
    this.boostrap ??= await bootstrap2();
...
var AiAgentService = class _AiAgentService {
  ...
  async initModel() {
    ...
    try {
      ...
      const { CreateMLCEngine } = await import("./chunk-YWW53EZW.mjs");
  ...
  async handleToolCall(toolCall) {
    ...
    switch (toolCall.tool) {
      case "list_services":
        result2 = "Available Services: iPhone Screen Repair, Android Battery Replacement, PC Optimization, Custom Web Development, SEO Consulting.";
        break;
      case "get_repair_status":
        const id = toolCall.args?.trackingId || "Unknown";
        result2 = `Repair ${id} is currently In Progress (Phase 2: Component Testing). ETA: 24hrs.`;
        break;
      case "book_appointment":
        result2 = `Appointment request received for ${toolCall.args?.serviceType} on ${toolCall.args?.date}. Please confirm via WhatsApp.`;
        break;
      default:
        result2 = "Tool not found or not supported.";
    }
    this.messages.update((msgs) => [
      ...msgs,
      { role: "assistant", content: `Checking... (Tool: ${toolCall.tool})` },
      { role: "user", content: `[Tool Output]: ${result2}` }
...
var routes = [
  __spreadValues({
    title: "Home",
    path: "",
    loadChildren: () => import("./chunk-ICWMREBM.mjs")
  }, true ? { \u0275entryName: "src/app/public/public.routes.ts" } : {}),
  __spreadValues({
    title: "Admin",
    path: "admin",
    loadChildren: () => import("./chunk-UWZSBIJZ.mjs")
  }, true ? { \u0275entryName: "src/app/admin/admin.routes.ts" } : {}),
  {
    path: "**",
    redirectTo: ""
  }
...
var SupabaseProductRepository = class _SupabaseProductRepository extends ProductRepository {
  ...
  findAvailable() {
    const fetchAll = async () => {
      ...
      `;
      while (hasMore) {
        const query = this.applyTenantFilter(this.supabase.from("products").select(select2)).eq("is_active", true).order("name").range(fromIdx, fromIdx + CHUNK - 1);
  ...
  getAll(branch_id) {
    const fetchAll = async () => {
      ...
      `;
      while (hasMore) {
        let query = this.supabase.from("products").select(select2);
  ...
  create(product) {
    const _a2 = product, { stock } = _a2, productData = __objRest(_a2, ["stock"]);
  ...
  upsertMany(products) {
    ...
    const dataToUpsert = products.map((p) => {
      const _a2 = p, { stock } = _a2, rest = __objRest(_a2, ["stock"]);
  ...
  updateMany(products) {
    ...
    const updates = products.map((p) => {
      ...
      const _a2 = p, { id, tenant_id } = _a2, rest = __objRest(_a2, ["id", "tenant_id"]);
      const cleanPayload = {};
...
var SupabaseRepairRepository = class _SupabaseRepairRepository extends RepairRepository {
  ...
  create(repair) {
    const _a2 = repair, { parts, images } = _a2, baseRepair = __objRest(_a2, ["parts", "images"]);
  ...
  mapToDb(entity) {
    const _a2 = entity, { device_brand, device_type, images } = _a2, rest = __objRest(_a2, ["device_brand", "device_type", "images"]);
...
var serverSupabase = createClient(environment.supabaseUrl, environment.supabaseKey, {
  auth: {
...
// src/main.server.ts
var bootstrap = (context) => bootstrapApplication(App, config2, context);
...
@angular/platform-server/fesm2022/platform-server.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-ULB6V7AR.mjs.map
