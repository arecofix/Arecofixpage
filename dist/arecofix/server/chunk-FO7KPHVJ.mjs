...
var AdminMessagesPage = class _AdminMessagesPage {
  ...
  async loadMessages() {
    ...
    const { data, error } = await supabase.from("contact_messages").select("*").eq("tenant_id", this.tenantService.getTenantId()).order("created_at", { ascending: false });
    if (data) {
  ...
  async markAsRead(id) {
    const supabase = this.auth.getSupabaseClient();
    const { error } = await supabase.from("contact_messages").update({ is_read: true }).eq("id", id).eq("tenant_id", this.tenantService.getTenantId());
    if (!error) {
  ...
  async deleteMessage(id) {
    if (!confirm("\xBFEst\xE1s seguro de eliminar este mensaje?"))
      return;
    const supabase = this.auth.getSupabaseClient();
    const { error } = await supabase.from("contact_messages").delete().eq("id", id).eq("tenant_id", this.tenantService.getTenantId());
    if (!error) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminMessagesPage, { className: "AdminMessagesPage", filePath: "src/app/admin/messages/admin-messages-page.ts", lineNumber: 13 });
})();
export {
  AdminMessagesPage
};
//# sourceMappingURL=chunk-FO7KPHVJ.mjs.map
