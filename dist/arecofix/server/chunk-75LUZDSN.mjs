...
var AdminBrandFormPage = class _AdminBrandFormPage {
  ...
  async ngOnInit() {
    ...
    if (this.id) {
      ...
      const { data, error } = await supabase.from("brands").select("*").eq("tenant_id", this.tenantService.getTenantId()).eq("id", this.id).single();
      if (data) {
        this.form.set({
          name: data.name,
          slug: data.slug,
  ...
  async save() {
    ...
    try {
      if (this.id) {
        const { error } = await supabase.from("brands").update(payload).eq("id", this.id).eq("tenant_id", this.tenantService.getTenantId());
        if (error)
          throw error;
      } else {
        const { error } = await supabase.from("brands").insert(__spreadProps(__spreadValues({}, payload), {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminBrandFormPage, { className: "AdminBrandFormPage", filePath: "src/app/admin/brands/admin-brand-form-page.ts", lineNumber: 14 });
})();
export {
  AdminBrandFormPage
};
//# sourceMappingURL=chunk-75LUZDSN.mjs.map
