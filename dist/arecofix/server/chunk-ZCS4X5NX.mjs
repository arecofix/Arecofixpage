...
var OrderService = class _OrderService {
  ...
  async updateOrderStatus(id, status) {
    const { error, data } = await this.supabase.from("orders").update({ status, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select();
    if (error)
  ...
  async updateOrder(id, order, items) {
    try {
      ...
      const { error: orderError, data: updatedOrdersData } = await this.supabase.from("orders").update(updatePayload).eq("id", id).select();
      if (orderError)
        throw orderError;
      if (!updatedOrdersData || updatedOrdersData.length === 0) {
        throw new Error("Supabase RLS Error: El servidor bloque\xF3 la actualizaci\xF3n de la Orden. Activa los permisos de edici\xF3n para 'orders' en Supabase.");
      }
      await this.handleItemsUpdate(id, items);
      const { data: fetchUpdatedData, error: fetchUpdatedError } = await this.supabase.from("orders").select("*").eq("id", id).single();
      if (fetchUpdatedError)
        throw fetchUpdatedError;
      const updatedDbOrder = fetchUpdatedData;
...
//# sourceMappingURL=chunk-ZCS4X5NX.mjs.map
