...
var TenantService = class _TenantService {
  ...
  _currentTenant = signal(null, ...ngDevMode ? [{ debugName: "_currentTenant" }] : []);
   ...
   */
  async resolveTenantByHostname(hostname) {
    this.logger.info(`Resolving tenant for hostname: ${hostname}`);
    try {
      let { data, error } = await this.supabase.from("tenants").select("*").eq("custom_domain", hostname).eq("is_active", true).maybeSingle();
      if (error) {
      ...
      if (!data) {
        ...
        const slugToSearch = hostname.includes("localhost") || hostname === "arecofix.com.ar" || hostname === "www.arecofix.com.ar" ? "arecofix" : extractSubdomain;
        this.logger.debug(`Searching for tenant by slug: ${slugToSearch}`);
        const { data: slugData, error: slugError } = await this.supabase.from("tenants").select("*").eq("slug", slugToSearch).eq("is_active", true).maybeSingle();
        if (slugError) {
        ...
        if (slugData) {
          ...
          const { data: fallbackData, error: fallbackError } = await this.supabase.from("tenants").select("*").limit(1).maybeSingle();
          if (fallbackError) {
...
var SupabaseService = class _SupabaseService {
  ...
  static \u0275fac = function SupabaseService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _SupabaseService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SupabaseService, factory: _SupabaseService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SupabaseService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var App = registerPlugin("App", {
  web: () => import("./chunk-XZEH2ICC.mjs").then((m) => new m.AppWeb())
});

// node_modules/@capacitor/browser/dist/esm/index.js
var Browser = registerPlugin("Browser", {
  web: () => import("./chunk-JWHG7BDS.mjs").then((m) => new m.BrowserWeb())
});
...
var AuthService = class _AuthService {
  ...
  async initAuth() {
    ...
    this.supabase.auth.onAuthStateChange(async (event, session2) => {
      if (session2) {
        const profile = await this.profileService.getProfile(session2.user.id);
        this.authState.next({ session: session2, user: session2.user, profile });
      } else {
  ...
  async signUp(email, password, profileData) {
    ...
    try {
      const response = await this.supabase.auth.signUp({
        ...
        password,
        options: {
          data: __spreadProps(__spreadValues({}, profileData), {
            tenant_id: tenantId
          })
        }
      });
...
//# sourceMappingURL=chunk-EIU5CNMA.mjs.map
