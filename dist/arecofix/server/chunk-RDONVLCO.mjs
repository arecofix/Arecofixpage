...
var AdminServiceFormPage = class _AdminServiceFormPage {
  ...
  async ngOnInit() {
    ...
    if (this.id) {
      ...
      const { data, error } = await supabase.from("services").select("*").eq("tenant_id", this.tenantService.getTenantId()).eq("id", this.id).single();
      if (data) {
        this.form.set({
          name: data.name,
  ...
  async save() {
    ...
    try {
      if (this.id) {
        const { error } = await supabase.from("services").update(payload).eq("id", this.id).eq("tenant_id", this.tenantService.getTenantId());
        if (error)
          throw error;
      } else {
        const { error } = await supabase.from("services").insert(__spreadProps(__spreadValues({}, payload), {
  ...
  }, dependencies: [FormsModule, \u0275NgNoValidate, DefaultValueAccessor, NumberValueAccessor, CheckboxControlValueAccessor, NgControlStatus, NgControlStatusGroup, RequiredValidator, MinValidator, NgModel, NgForm, RouterLink], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AdminServiceFormPage, [{
    type: Component,
    args: [{ selector: "app-admin-service-form-page", standalone: true, imports: [FormsModule, RouterLink], template: `<div class="max-w-2xl mx-auto">\r
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminServiceFormPage, { className: "AdminServiceFormPage", filePath: "src/app/admin/services/admin-service-form-page.ts", lineNumber: 14 });
})();
export {
  AdminServiceFormPage
};
//# sourceMappingURL=chunk-RDONVLCO.mjs.map
