...
var AdminCategoryFormPage = class _AdminCategoryFormPage {
  ...
  async ngOnInit() {
    ...
    const { data: categoriesData } = await supabase.from("categories").select("id, name").eq("tenant_id", this.tenantService.getTenantId()).eq("is_active", true).order("name");
    if (categoriesData) {
      this.categories.set(categoriesData);
    }
    if (this.id) {
      const { data, error } = await supabase.from("categories").select("*").eq("tenant_id", this.tenantService.getTenantId()).eq("id", this.id).single();
      if (data) {
        this.form.set({
          name: data.name,
          slug: data.slug,
  ...
  async save() {
    ...
    try {
      if (this.id) {
        const { error } = await supabase.from("categories").update(payload).eq("id", this.id).eq("tenant_id", this.tenantService.getTenantId());
        if (error)
          throw error;
      } else {
        const { error } = await supabase.from("categories").insert(__spreadProps(__spreadValues({}, payload), {
  ...
  }, dependencies: [FormsModule, \u0275NgNoValidate, NgSelectOption, \u0275NgSelectMultipleOption, DefaultValueAccessor, CheckboxControlValueAccessor, SelectControlValueAccessor, NgControlStatus, NgControlStatusGroup, RequiredValidator, NgModel, NgForm, RouterLink], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AdminCategoryFormPage, [{
    type: Component,
    args: [{ selector: "app-admin-category-form-page", standalone: true, imports: [FormsModule, RouterLink], template: `<div class="max-w-4xl mx-auto text-base-content">\r
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminCategoryFormPage, { className: "AdminCategoryFormPage", filePath: "src/app/admin/categories/admin-category-form-page.ts", lineNumber: 14 });
})();
export {
  AdminCategoryFormPage
};
//# sourceMappingURL=chunk-S6NCLHN2.mjs.map
