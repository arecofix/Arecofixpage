...
var AdminProductService = class _AdminProductService {
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (productsToUpsert.length > 0) {
            const { error } = await this.supabase.from("products").upsert(productsToUpsert, { onConflict: "id" });
            if (error)
              throw error;
          }
  ...
  async bulkUpdate(ids, payload) {
    const { error } = await this.supabase.from("products").update(payload).in("id", ids);
    if (error)
      throw error;
  }
  async bulkIncreasePrice(ids, percentage) {
    const { data: products, error } = await this.supabase.from("products").select("id, price").in("id", ids);
    if (error)
      throw error;
    ...
    const updates = products.map((p) => ({
      ...
      // Round to integer for simplicity, or 2 decimals
    }));
    const BATCH_SIZE = 5;
    for (let i = 0; i < updates.length; i += BATCH_SIZE) {
      const batch = updates.slice(i, i + BATCH_SIZE);
      await Promise.all(batch.map((u) => this.supabase.from("products").update({ price: u.price }).eq("id", u.id)));
    }
  }
...
//# sourceMappingURL=chunk-SGZTEKE5.js.map
