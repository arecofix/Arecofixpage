...
var AdminInvoiceDetailPage = class _AdminInvoiceDetailPage {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      if (id) {
        const { data: invoice, error: invoiceError } = await supabase.from("invoices").select("*").eq("id", id).eq("tenant_id", this.tenantService.getTenantId()).single();
        if (invoiceError)
          throw invoiceError;
        if (invoice) {
          this.invoice.set(invoice);
          if (invoice.sale_id) {
            const { data: items, error: itemsError } = await supabase.from("sale_items").select("*, products(name)").eq("sale_id", invoice.sale_id).eq("tenant_id", this.tenantService.getTenantId());
            if (itemsError) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminInvoiceDetailPage, { className: "AdminInvoiceDetailPage", filePath: "src/app/admin/sales/admin-invoice-detail-page.ts", lineNumber: 14 });
})();
export {
  AdminInvoiceDetailPage
};
//# sourceMappingURL=chunk-LBA5ND7X.js.map
